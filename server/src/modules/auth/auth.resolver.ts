...
export class AuthResolver {
	...
	@Mutation(() => String)
	public async checkByFacebook(@Args('fbAccessToken') fbAccessToken: string) {
		const token = await this.authService.checkByFacebook(fbAccessToken);
		return { token };
	}
	...
	public async sendVerificationEmail(
		...
		try {
			...
			const result = await this.authService.sendVerificationEmail(input);
